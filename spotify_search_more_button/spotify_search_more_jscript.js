    ...
    if(move=='Enter'){
        ...
        $.get(searchURL, function(data){
            ...
            if(data[ddVal].next) {
                ...
                htmlMore += '<button class="additional">&gt;&gt;More Results&gt;&gt;</button>';
                moreButton = $('#more').eq(0);
                moreButton.html(htmlMore);
            }
        });
    }
});
...
moreButton.on('click', function() {
    ...
    $.get(moreURL, function(data){
        ...
        if(data[ddVal].next) {
            moreURL = data[ddVal].next;
            htmlMore += '<button class="additional">&gt;&gt;Next Page&gt;&gt;</button>';
            moreButton = $('#more').eq(0);
            moreButton.html(htmlMore);
        } else {
            htmlMore ='';
